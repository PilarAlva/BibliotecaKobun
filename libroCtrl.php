...
class libroCtrl extends Controlador{
    ...
    public function busqueda($filtro = '', $busqueda = '', $pagina = '1'){
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            $busqueda_url = rawurlencode($busqueda);
$filtro_url = rawurlencode($filtro);

            header('Location: ' . BASE_URL . 'catalogo/b/' . $filtro_url . '/' . $busqueda_url);
exit;

        }

        $busqueda = rawurldecode($busqueda);
$filtro = rawurldecode($filtro);
...
$filtro = str_replace('_', ' ', $filtro);
    ...
    private function urlPaginacion($filtro, $busqueda, $pagina) {
        ...
        $filtro_url = str_replace(' ', '_', $filtro);
    $busqueda_url = str_replace(' ', '_', $busqueda);

        return BASE_URL . 'catalogo/b/' . str_replace(' ', '_', $filtro) . '/' . str_replace(' ', '_', $busqueda) . '/';
    }
